...
public class BlackBoxDiv2_2 {
    ...
    private int addPerm(int[][] a, int i, int j) {
        long res = 0;
        for (int k = 1; k < i; k++) {
            res += correctModulo((long) correctModulo((long) Math.pow(2, i - k))
                    * (long) correctModulo((long) a[i - k][j - 1]
                    * (long) binomialCoefficient(i, i - k)));
            res = correctModulo(res);
        }
    ...
    public int count(String front, String side) {
        ...
        for (int i = 2; i < n; i++) {
            for (int j = i; j < m; j++) {
                a[i][j] = correctModulo((long) correctModulo(a[i][j - 1]
                        * ((long) correctModulo((long) Math.pow(2, i) - 1)))
                        + (long) (addPerm(a, i, j)));
                if (j < n) {
                    a[j][i] = a[i][j];
                }
